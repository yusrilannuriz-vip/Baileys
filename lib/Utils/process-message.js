...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, keyStore, logger, options, getMessage }) => {
    ...
    // Do not convert a LID into a fake phone JID by replacing @lid with @s.whatsapp.net.
    // A LID is not the phone number. If messages-recv could not resolve it, preserve the
    // LID so the Base layer can resolve it from its own LID/JID mapping cache.
    const mJids = content && content.contextInfo && content.contextInfo.mentionedJid ? content.contextInfo.mentionedJid : [];
    // Keep unresolved LIDs in mentionedJid/contextInfo as LIDs. Converting
    // `123@lid` to `123@s.whatsapp.net` fabricates a phone-number identity.
