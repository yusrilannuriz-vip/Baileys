    ...
    for (let i = 0; i < text.length; i++) {
        if (text[i] === '[' && text[i - 1] !== '\\') {
            ...
            if (type === 'latex') {
                ...
                key = `VELYX_HYPERLINK_${hyperlink_index++}`;
                tag = `{{${key}}}${url}{{/${key}}}`;
                data = { type: 'hyperlink', ie: { key, text: raw, url, is_trusted: trusted } };
            } else {
                if (!citation) continue;
                key = `VELYX_CITATION_${citation_index - 1}`;
                tag = `{{${key}}}${url}{{/${key}}}`;
...
const normalizeRichInlineEntities = (items) => {
    if (!Array.isArray(items)) return [];
    return items.map(item => {
        if (item?.metadata && item?.key) return item;
        if (item?.latex) {
            const l = item.latex;
            return {
                key: item.key || `VELYX_LATEX_${randomUUID()}`,
                metadata: {
                    latex_expression: l.expression || l.latex_expression || '',
                    latex_image: {
                        url: l.url || l.image_url || '',
                        width: Number(l.width) || 100,
                        height: Number(l.height) || 100
                    },
                    font_height: Number(l.font_height) || 83.333,
...
const normalizeRichImageV2 = (input) => {
    const image = typeof input === 'string' ? { url: input } : (input || {});
    const url = image.url || image.image_url;
    if (!url) throw new TypeError('richMessage.imgv2 must contain url');
    return {
        url,
        preview: image.preview_url || image.preview_image_url || image.full_image_url || url,
        full: image.full_url || image.high_res_url || image.full_image_url || url,
        mime: image.mime_type || image.mimeType || 'image/jpeg',
        width: image.width,
        height: image.height
    };
};

const makeRichHScrollPrimitive = (card) => ({
    ...
    body: {
        __typename: card?.body_typename || 'GenAI3PExtCalendarEventList',
        sections: Array.isArray(card?.sections) ? card.sections : [],
        ctas: (card?.buttons || card?.ctas || []).map(cta => ({
            label: cta?.label || cta?.cta_text || '',
            state: cta?.state || 'PENDING',
            kind: cta?.kind || 'OTHER',
            tool_call_id: cta?.tool_call_id || randomUUID(),
            ...(cta?.toast || cta?.toast_label ? {
                toast: {
                    label: cta.toast?.label || cta.toast_label || '',
...
const makeRichFooterPrimitive = (action) => {
    if (action?.text !== undefined) {
        return {
            text: String(action.text),
            ...(action.inline_entities ? {
                inline_entities: normalizeRichInlineEntities(action.inline_entities)
            } : {}),
            __typename: 'GenAIMarkdownTextUXPrimitive'
        };
    }
    return {
        cta_text: action?.cta_text || action?.label || '',
        cta_type: action?.cta_type || 'OPEN_URL',
        cta_url: action?.cta_url || action?.url || '',
        ...(action?.cta_color || action?.color ? {
            cta_color: action.cta_color || action.color
        } : {}),
...
const generateWAMessageContent = async (message, options) => {
            ...
            viewOnceMessage: {
                message: {
                    ...
                    interactiveMessage: WAProto_1.proto.Message.InteractiveMessage.create({
                        ...
                        footer: {
                            text: os.footer || 'Powered by Velyx'
                        },
                        nativeFlowMessage: {
                            buttons: [
                                {
    ...
    const editrich = rich?.editrich && typeof rich.editrich === 'object'
        ? { ...rich.editrich }
        : null;
    if (editrich) delete editrich.delay;
    const submessages = [];
    const sections = [];
    ...
    if (rich.text) {
        ...
        const inline_entities = [
            ...(parsed.ie ? parsed.ie.map(({ type, ie }) => {
                if (type === 'hyperlink') return {
                    key: ie.key,
                    metadata: {
                        display_name: ie.text,
            ...
            ...normalizeRichInlineEntities(rich.inline_entities)
        ];
    ...
    if (rich.html) {
        const html = typeof rich.html === 'string' ? rich.html : String(rich.html);
        const htmlText = rich.text || rich.title || 'Rich HTML';
    ...
    if (rich.image) {
        ...
        submessages.push({
            ...
            gridImageMetadata: {
                ...
                imageUrls: [{ imagePreviewUrl: url, imageHighResUrl: url, sourceUrl: 'https://github.com/yusrilannuriz-vip/Baileys' }]
            }
        });
        sections.push(newLayout('Single', {
    ...
    if (rich.images) {
        ...
        const imageUrls = urls.map(url => ({
            imagePreviewUrl: url,
            imageHighResUrl: url,
            sourceUrl: 'https://github.com/yusrilannuriz-vip/Baileys'
        }));
        submessages.push({
            messageType: 1,
            gridImageMetadata: {
    ...
    const richImageV2 = rich.imgv2 !== undefined ? rich.imgv2 : rich.imageV2;
    if (richImageV2 !== undefined) {
        const image = normalizeRichImageV2(richImageV2);
        submessages.push({
            messageType: 1,
            gridImageMetadata: {
                gridImageUrl: { imagePreviewUrl: image.preview },
                imageUrls: [{
                    imagePreviewUrl: image.preview,
                    imageHighResUrl: image.full,
                    sourceUrl: image.url
                }]
            }
        });
        ...
        if (image.width !== undefined) {
            preview_image.width = Number(image.width);
            full_image.width = Number(image.width);
        }
        if (image.height !== undefined) {
            preview_image.height = Number(image.height);
            full_image.height = Number(image.height);
        }
    ...
    const cleanHScroll = rich.hscroll !== undefined ? rich.hscroll : rich.hscrollV2;
    if (cleanHScroll !== undefined) {
        const cards = Array.isArray(cleanHScroll) ? cleanHScroll : [cleanHScroll];
        sections.push({
            __typename: 'GenAIUnifiedResponseSection',
            view_model: {
                __typename: 'GenAIHScrollLayoutViewModel',
                primitives: cards.map(makeRichHScrollPrimitive)
            }
        });
    }

    const cleanFooter = rich.footerActionV2 !== undefined
        ? rich.footerActionV2
        : (Array.isArray(rich.footer) || typeof rich.footer === 'object' ? rich.footer : null);

    if (cleanFooter) {
        const actions = Array.isArray(cleanFooter) ? cleanFooter : [cleanFooter];
        sections.push({
            view_model: {
                __typename: 'GenAIActionRowLayoutViewModel',
                primitives: actions.map(makeRichFooterPrimitive)
            }
        });
    }
    ...
    if (rich.post) {
        ...
        sections.push(newLayout("HScroll", posts.map(p => ({
            ...
            post_url: p.post_url || "https://github.com/yusrilannuriz-vip/Baileys",
            post_deeplink: p.post_deeplink || p.post_url || "https://github.com/yusrilannuriz-vip/Baileys",
    ...
    if (typeof rich.footer === 'string' && rich.footer) sections.push(newLayout('Single', { text: rich.footer, __typename: 'GenAIMetadataTextPrimitive' }));

    const unifiedData = {
        response_id: rich.response_id || randomUUID(),
    ...
    if (editrich) {
        Object.defineProperty(m, '__richEdit', {
            value: {
                delay: Math.max(0, Number(rich.editrich.delay) || 0),
                content: editrich
            },
            enumerable: false,
            configurable: true
...
const generateWAMessage = async (jid, content, options = {}) => {
    var _a;
    options.logger = (_a = options?.logger)?.child?.({ msgId: options.messageId });
    ...
    const richEdit = generatedContent?.__richEdit;
    if (richEdit) {
        try { delete generatedContent.__richEdit; } catch (_) {}
    }

    const result = generateWAMessageFromContent(jid, generatedContent, options);

    if (richEdit) {
        const conn = options.conn || options.socket || globalThis.__VELYX_RICH_CONN__;
        if (conn && typeof conn.relayMessage === 'function') {
            Promise.resolve().then(async () => {
                if (richEdit.delay > 0) {
                    await new Promise(resolve => setTimeout(resolve, richEdit.delay));
                }

                const editedContent = await generateWAMessageContent(
                    { richMessage: richEdit.content },
                    { newsletter: (0, WABinary_1.isJidNewsletter)(jid), ...options }
                );
                ...
                options.logger?.error?.({ err: error }, 'richMessage edit failed');
            });
        }
    }
...
const toJid = (id) => {
    if (!id) return '';
    // LID is an identity namespace, not a phone number. Never fabricate a
    // @s.whatsapp.net JID by changing the suffix.
...
const setRichMessageConnection = (conn) => {
    globalThis.__VELYX_RICH_CONN__ = conn;
    return conn;
};
...
exports.setRichMessageConnection = setRichMessageConnection;
