...
[![GitHub](https://img.shields.io/badge/GitHub-yusrilannuriz--vip%2FBaileys-181717?logo=github)](https://github.com/yusrilannuriz-vip/Baileys)
[![License](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)
</div>

<div align="center">
  <strong>Velyx</strong> — WhatsApp API Multi‑Device Modern untuk bot dan aplikasi berbasis Node.js.
</div>
...
```bash
npm install github:yusrilannuriz-vip/Baileys
```

Atau dengan Yarn:

```bash
yarn add github:yusrilannuriz-vip/Baileys
```

Untuk memasang branch `main` secara eksplisit:

```bash
npm install github:yusrilannuriz-vip/Baileys#main
```

> Velyx didistribusikan melalui repository GitHub ini, bukan melalui package npm bernama `velyx`.

Cara Impor

```javascript
// CommonJS
const { default: makeWASocket } = require("velyx")

// ES6 Module
import makeWASocket from "velyx"
```
...
```javascript
const { default: makeWASocket, DisconnectReason, useMultiFileAuthState } = require('velyx')
async function connectToWhatsApp() {
    // Simpan kredensial di folder 'auth_info_velyx'
    const { state, saveCreds } = await useMultiFileAuthState('auth_info_velyx')
    
    const sock = makeWASocket({
        auth: state,
        printQRInTerminal: true,
        browser: ['Velyx', 'Desktop', '3.0']
    })
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const m of messages) {
            ...
            await sock.sendMessage(m.key.remoteJid, { 
                text: 'Halo! Saya bot WhatsApp menggunakan velyx 🤖' 
            })
        }
    })
...
WhatsApp menyediakan API multi‑device yang memungkinkan velyx terautentikasi sebagai klien WhatsApp sekunder melalui QR code atau pairing code.

Hubungkan dengan QR Code

```javascript
const { default: makeWASocket, Browsers } = require("velyx")
...
```javascript
const { default: makeWASocket, useMultiFileAuthState } = require("velyx")
...
velyx tidak menyertakan penyimpanan default untuk obrolan, kontak, atau pesan. Namun, implementasi in‑memory sederhana disediakan.

```javascript
const { default: makeWASocket, makeInMemoryStore } = require("velyx")

const store = makeInMemoryStore({ })

store.readFromFile('./velyx_store.json')

setInterval(() => {
    store.writeToFile('./velyx_store.json')
}, 10_000)
...
await sock.sendMessage(
    jid,
    {
        text: 'Halo, ini dikirim menggunakan https://github.com/yusrilannuriz-vip/Baileys'
    }
)
```
...
import { downloadMediaMessage, getContentType } from 'velyx'
...
Didistribusikan di bawah **MIT License**. Lihat file `LICENSE` untuk informasi lengkap.

Velyx merupakan modifikasi/fork dari proyek Baileys dan tetap mempertahankan atribusi upstream WhiskeySockets sesuai ketentuan lisensi MIT.

---
<div align="center">
  <strong>Velyx</strong> — velyx<br>
